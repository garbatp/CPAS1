...
void CPAS_CGH_3DPS_2d(int Np, int* xo, int* yo, int* zo, int* uo, int Nx, int Ny, int dx, float lambda, int S_Bx, int S_By, cuComplex* fths)
{
...
*/
	...
	calculate << < grid, block >> >(fths_p, d_xo, d_yo, uo, d_z02, dfxs, lambda, k0, Ts, d_fxs, d_y0seg, d_x0seg, S_Bx, S_Bx, 128, 128);
...
int main()
{
	...
	CPAS_CGH_3DPS_2d(Np, xo, yo, zo, uo, Nx, Ny, dx, lambda, S_Bx, S_By, fhs);
	...
	/*START CUDA FFT PART */
	execute2D(&forwardPlan, fhs, holo, CUFFT_FORWARD); 

	cudaEventRecord(stop, 0);
	cudaEventSynchronize(stop);

	cudaMemcpy(h_out, holo, sizeof(cufftComplex)*S_Bx*S_By*batch, cudaMemcpyDeviceToHost); //Kopiowanie wyniku do pamiêci CPU
